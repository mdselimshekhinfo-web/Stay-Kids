...
async function run() {
  ...
  const setupReq = await api("/action", "POST", { type: "add-child", newChild: { id: "child-1", name: "Child Phone", device: "Emulator" } }, tok);
  console.log("Add child res:", setupReq.data);
  const s0 = await api("/state", "GET", null, tok);
  console.log("State:", JSON.stringify(s0.data.children));
}
run();
